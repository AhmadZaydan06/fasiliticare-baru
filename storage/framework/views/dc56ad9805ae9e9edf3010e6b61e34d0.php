...
<div class="main-content" style="margin-left: 0; padding: 0; width: 100%;">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 border-0 shadow-sm" style="border-left: 5px solid #003366 !important;">
                ...
                <h2 class="fw-bold m-0 mt-1"><?php echo e($totalAduan); ?></h2>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 border-0 shadow-sm" style="background-color: #fff8e1; border-left: 5px solid #ffc107 !important; color: #856404;">
                ...
                <h2 class="fw-bold m-0 mt-1"><?php echo e($menunggu); ?></h2>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 border-0 shadow-sm" style="background-color: #e3f2fd; border-left: 5px solid #2196f3 !important; color: #0c5460;">
                ...
                <h2 class="fw-bold m-0 mt-1"><?php echo e($diproses); ?></h2>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 border-0 shadow-sm" style="background-color: #e8f5e9; border-left: 5px solid #4caf50 !important; color: #155724;">
                ...
                <h2 class="fw-bold m-0 mt-1"><?php echo e($selesai); ?></h2>
            </div>
        </div>
    </div>
    ...
    <div class="table-container p-4 bg-white shadow-sm" style="border-radius: 15px;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <form action="<?php echo e(route('admin.dashboard')); ?>" method="GET" class="search-bar d-flex align-items-center gap-2 border px-3 py-1" style="border-radius: 10px; width: 350px;">
                <i class="fas fa-search text-muted"></i>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari penghuni, kamar, atau kategori..." style="border: none; outline: none; font-size: 14px; width: 100%;">
            </form>
            
            
            <div class="d-flex align-items-center gap-2">
                <i class="fas fa-filter text-muted me-2" style="font-size: 14px;"></i>
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="btn btn-sm <?php echo e(!request('status') ? 'btn-primary' : 'btn-light'); ?> rounded-pill px-3">Semua</a>
                <a href="<?php echo e(route('admin.dashboard', ['status' => 1])); ?>" class="btn btn-sm <?php echo e(request('status') == 1 ? 'btn-primary' : 'btn-light'); ?> rounded-pill px-3">Menunggu</a>
                <a href="<?php echo e(route('admin.dashboard', ['status' => 2])); ?>" class="btn btn-sm <?php echo e(request('status') == 2 ? 'btn-primary' : 'btn-light'); ?> rounded-pill px-3">Diproses</a>
                <a href="<?php echo e(route('admin.dashboard', ['status' => 3])); ?>" class="btn btn-sm <?php echo e(request('status') == 3 ? 'btn-primary' : 'btn-light'); ?> rounded-pill px-3">Selesai</a>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-borderless m-0">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $semuaAduan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                <div class="profile-avatar bg-primary-subtle text-primary fw-bold d-flex justify-content-center align-items-center" style="width: 40px; height: 40px; border-radius: 50%;">
                                    <?php echo e($row->anggota ? substr($row->anggota->nama_lengkap, 0, 1) : '?'); ?>

                                </div>
                                <div>
                                    <div class="fw-bold" style="font-size: 14px;"><?php echo e($row->anggota->nama_lengkap ?? 'Data Tidak Ada'); ?></div>
                                    <div class="text-muted" style="font-size: 12px;"><i class="fas fa-bed me-1"></i> Kamar <?php echo e($row->anggota->nomor_kamar ?? '-'); ?></div>
                                </div>
                            </div>
                        </td>
                        <td>
                            ...
                            <span class="text-muted small"><?php echo e(Str::limit($row->deskripsi_masalah, 60)); ?></span>
                        </td>
                        <td class="text-muted" style="font-size: 12px;">
                            <div><i class="far fa-calendar-alt me-1"></i> <?php echo e(date('d M Y', strtotime($row->waktu_aduan))); ?></div>
                            <div><i class="far fa-clock me-1"></i> <?php echo e(date('H:i', strtotime($row->waktu_aduan))); ?></div>
                        </td>
                        <td>
                            <select class="form-select form-select-sm status-dropdown-ajax 
                                <?php echo e($row->id_status == 1 ? 'status-menunggu' : ($row->id_status == 2 ? 'status-diproses' : 'status-selesai')); ?>"
                                data-id="<?php echo e($row->id_aduan); ?>" style="border-radius: 20px; font-weight: 600; font-size: 13px;">
                                <option value="1" <?php echo e($row->id_status == 1 ? 'selected' : ''); ?>>Menunggu</option>
                                <option value="2" <?php echo e($row->id_status == 2 ? 'selected' : ''); ?>>Diproses</option>
                                <option value="3" <?php echo e($row->id_status == 3 ? 'selected' : ''); ?>>Selesai</option>
                            </select>
                        </td>
                        <td style="text-align: center;">
                            <div class="d-flex justify-content-center gap-2">
                                <button class="btn btn-link text-muted p-0"><i class="fas fa-external-link-alt"></i></button>
                                
                                
                                <form action="<?php echo e(route('admin.aduan.destroy', $row->id_aduan)); ?>" method="POST" onsubmit="return confirm('Hapus aduan ini?')">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\fasiliticare-ta\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>